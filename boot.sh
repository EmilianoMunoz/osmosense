...
LOG_DIR="${LOG_DIR:-data/logs/boot}"
PID_DIR="${PID_DIR:-data/state}"
...
setup_postgis() {
    local args=(venv/bin/python scripts/setup_postgis_local.py)
...
run_smoke() {
    ...
    API_BASE_URL="http://${API_HOST}:${API_PORT}" \
        DATABASE_URL="${DATABASE_URL:-$DEFAULT_DATABASE_URL}" \
        venv/bin/python scripts/smoke_test_operativo.py --require-source postgis --check-postgis
}
