...
PARCELAS_VID_OLIVO = "backend/data/parcelas/san_rafael_vid_olivo_wgs84.geojson"
PARCELAS_COMPLETO = "backend/data/parcelas/san_rafael_completo_wgs84.geojson"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-usuarios",
        action="store_true",
        help="Omite la carga de usuarios demo.",
    )
    parser.add_argument(
        "--all-parcelas",
        action="store_true",
...
def main() -> None:
    ...
    commands = [
        [sys.executable, "backend/scripts/postgis/aplicar_schema_postgis.py", *common],
        parcelas_command,
        [sys.executable, "backend/scripts/postgis/cargar_ranking_postgis.py", *common],
        [sys.executable, "backend/scripts/postgis/cargar_zonificacion_um_postgis.py", *common],
    ]
    if not args.skip_clientes:
        commands.insert(
            3,
            [sys.executable, "backend/scripts/postgis/cargar_clientes_parcelas_postgis.py", *common],
        )
    if not args.skip_usuarios:
        commands.append(
            [sys.executable, "backend/scripts/postgis/cargar_usuarios_demo_postgis.py", *common]
        )
