...
def render_client_metrics(df: pd.DataFrame) -> None:
    ...
    col3.metric("Prioridad crítica", int((df[priority_col] == "critica").sum()))
    col4.metric("Prioridad alta", int((df[priority_col] == "alta").sum()))

    col5, col6, col7, col8 = st.columns(4)
    col5.metric("Riesgo promedio", f"{riesgo_prom:.1f}")
    col6.metric("Riesgo máximo", f"{riesgo_max:.1f}" if pd.notna(riesgo_max) else "-")
